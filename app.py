    ...
    if not df_v.empty:
        ...
        if clientes_lista:
            ...
            if 0.00 <= saldo_real_neto <= 0.01:
                ...
                # 🧮 1. TU LÓGICA ORIGINAL DE SUMA TOTAL NETA
                saldo_real_neto = round(df_cli['MONTO($)'].sum(), 2)
                ...
                total_abonos_historicos = abs(sum(float(m['MONTO($)']) for m in df_cli.to_dict('records') if m['TIPO'].strip().lower() in ['abono']))

                # 📊 2. PANTALLA PRINCIPAL
                c1, c2 = st.columns(2)
                if saldo_real_neto <= 0.01:
                   c1.metric("TOTAL ABONADO (CICLO ACTIVO)", "$0.00")
                   c2.metric("SALDO PENDIENTE NETO", "$0.00")
                   st.write("---")
                   st.success("🟢 Este cliente está al día. Ambos marcadores están en $0.00")
                   ...
                   recibo_texto = "El cliente se encuentra al día con sus cuentas."
                    ...
                    # 1. Convertimos el historial a una lista de diccionarios cronológica (de la más vieja a la más nueva)
                    movimientos_cliente = df_cl1.to_dict('records')
        
                    saldo_cronologico = 0.0
                    lineas_pantalla_limpia = []
          
                    # Recorremos en orden natural para construir el historial interactivo
                    for mov in movimientos_cliente:
                        ...
                        fecha_factura = fecha_completa[:10] if " " in fecha_completa else fecha_completa
            
                        # Si la cuenta llegó a cero absoluto en el paso anterior, reiniciamos la lista visible
                        if saldo_cronologico == 0.0 and monto > 0:
                            lineas_pantalla_limpia = []
            
                        # Procesamos según el tipo de movimiento
                        if tipo_mov == 'crédito' or tipo_mov == 'credito':
                            saldo_cronologico += monto
                            lineas_pantalla_limpia.append(f"📅 {fecha_factura} | Crédito: ${monto:.2f}")
                        elif tipo_mov == 'abono':
                            saldo_cronologico -= monto
                            lineas_pantalla_limpia.append(f"💰 {fecha_factura} | Abono: -${monto:.2f}")
                
                        # Corrección por decimales flotantes de Python
...
def formulario_simulador_costos():
    ...
    # 🟢 1. RECOGER EL EXCEL DE LA MEMORIA DE LA SESIÓN
    df_costos_real = st.session_state.get("df_costos_real", pd.DataFrame())

    ingredientes_modificados = {}

    # Creamos la interfaz en dos columnas para el teléfono
    ...
    # 🟢 2. CÁLCULO MATEMÁTICO UNIFICADO (Ocurre ABAJO, después de tener todas las variables cargadas)
    costo_materia_prima_total = 0.0
    
    # Limpieza previa del Excel si existe
    ...
    for ingrediente, cant_actual in ingredientes_modificados.items():
        costo_unitario = 1.0  # Valor por defecto modificado para tus pruebas
