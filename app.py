    ...
    if not df_v.empty:
        ...
        if clientes_lista:
           ...
           # cliente_sel = st.selectbox("Ver deudor específico:", clientes_lista, key="cobrar_cliente_sel")
            ...
            # --- 1. USAMOS TU BASE ORIGINAL 'df_v' QUE YA FUNCIONABA PERFECTO ---
            df_cli = df_v[df_v['CLIENTE'] == cliente_sel].copy()
  
            # Calculamos el saldo real neto con todo el historial completo del cliente
            saldo_real_neto = round(df_cli['MONTO($)'].sum(), 2)

            # --- 2. EVALUAMOS SI DEBE O ESTÁ AL DÍA ---
            if 0.00 <= saldo_real_neto <= 0.01:
                ...
                c1, c2 = st.columns(2)
                c1.metric("TOTAL ABONADO (DEUDA ACTUAL)", f"${abs(saldo_real_neto):.2f}")
                c2.metric("SALDO PENDIENTE NETO", f"${abs(saldo_real_neto):.2f}")
                st.write("---")
                st.warning(f"🟠 El cliente presenta un saldo pendiente de ${abs(saldo_real_neto):.2f}")

            # Totales históricos de la base de datos
            movimientos_dict = df_cli.to_dict('records')
            total_creditos = sum(float(m.get('MONTO($)', 0)) for m in movimientos_dict if str(m.get('TIPO', '')).strip().lower() == 'crédito')
            total_abonos_historicos = abs(sum(float(m.get('MONTO($)', 0)) for m in movimientos_dict if str(m.get('TIPO', '')).strip().lower() == 'abono'))

            st.write("---")
            st.write("### ⚙️ 1. MOTOR INVERSO (CORTE PERFECTO DE CICLO)")
            st.write("---")
            ...
            # --- 3. RECUADRO INTERACTIVO CON PAGINACIÓN VISUAL ---
            st.write("---")
            st.write("### 📋 Historial Actual de la Cuenta")

            if historial_recuadro:
                df_mostrar_completo = pd.DataFrame(historial_recuadro)[['FECHA', 'TIPO', 'MONTO($)']]
        
                # Paginación local de 10 en 10 para soportar la página 11+ sin romper nada
                total_registros = len(df_mostrar_completo)
                ...
                df_mostrar = df_mostrar_completo.iloc[start:end]
